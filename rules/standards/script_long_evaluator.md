# SCRIPT — EVALUATOR (gate, long-form narration)

Strict inspection. Same three dimensions as every script gate — delivery, the
addiction loop, voice — judged for the LONG-FORM format. Jargon is a penalty.

## SCORING (hybrid; totals 100, minus penalty)

| Type | Check | Points |
|---|---|---|
| graded | **D1 Delivery** — carries the stakes, the theme's belief-shift, the structure's arc, and the hook (verify against THE STORY PACKAGE above); the payoff lands the WHY beneath the event — the desire and the belief driving it — not just the surface event (a payoff that shows only WHAT happened scores low) | 0–20 |
| binary | **L1 Hook opens a loop** — a spine question opens early | 0 or 12 |
| binary | **L2 Head fake** — at least one earned reveal that breaks a prediction | 0 or 12 |
| graded | **L3 Loops chain / no dead air** — nested loops keep the piece moving across its length | 0–13 |
| graded | **V1 Immersive narration** — flowing, breathing, builds; lets moments land (NOT clipped) | 0–25 |
| graded | **V2 Visceral & concrete** — paints the scene, specific, present | 0–18 |
| **penalty** | **P1 Jargon** — clinical/AI words present | **−15** |

Sum the points, then subtract the jargon penalty.

## VERDICT (floor)

- **PASS if the score clears the floor (60)**; iterate toward target.
- **REJECT below the floor** — it doesn't deliver, doesn't hook, or sags (dead air).

## OUTPUT (use exactly)

```
VERDICT: PASS | REJECT
D1 delivery (/20): ____
L1 hook opens loop (0/12): ____
L2 head fake (0/12): ____
L3 loops chain (/13): ____
V1 immersive narration (/25): ____
V2 visceral & concrete (/18): ____
JARGON present (true/false): ____
TOTAL after penalty (/100): ____
Why: <one line>
If REJECT — failed checks: <numbers + one line each>
```
